...
async function toggleDone(id) {
  ...
  if (!t.done && t.repeat && t.due) {
    ...
    if (typeof addHistoryEntry === 'function') {
      await addHistoryEntry(t.done ? 'completed' : 'reopened', t);
    }
    markDirty();
    render();
    if (t.done) {
...
async function deleteTask(id) {
  ...
  // Registrar en historial ANTES de borrar de la BD
  if (typeof addHistoryEntry === 'function' && deleted) {
    await addHistoryEntry('deleted', deleted);
  }
...
(async function init() {
  ...
  window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', updateActiveTheme);
  if (navigator.getBattery) {
    navigator.getBattery().then(battery => {
      battery.addEventListener('levelchange', updateActiveTheme);
      battery.addEventListener('chargingchange', updateActiveTheme);
    }).catch(() => {});
  }
  await loadTasks();
  ...
  // Inicializar sincronización con GitHub y verificar versiones
  initGithubSync();
})();
...
async function updateActiveTheme() {
  const config = await dbGetMeta('themeConfig') || {
    theme: 'auto',
    autoType: 'system',
    batteryThreshold: 20,
    scheduleLightStart: '08:00',
    scheduleDarkStart: '20:00'
  };
  ...
  if (config.theme === 'dark') {
    ...
    if (config.autoType === 'system') {
      resolvedTheme = window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
    } else if (config.autoType === 'battery') {
      if (navigator.getBattery) {
        try {
          const battery = await navigator.getBattery();
          const pct = battery.level * 100;
          resolvedTheme = pct <= config.batteryThreshold ? 'dark' : 'light';
        } catch (e) {
          resolvedTheme = window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
        }
      } else {
    ...
    } else if (config.autoType === 'schedule') {
      const now = new Date();
      ...
      const darkStart = parseTime(config.scheduleDarkStart || '20:00');
      const lightStart = parseTime(config.scheduleLightStart || '08:00');
      
      if (darkStart > lightStart) {
        if (currentMin >= darkStart || currentMin < lightStart) {
          resolvedTheme = 'dark';
        } else {
          resolvedTheme = 'light';
        }
  ...
  try {
    localStorage.setItem('app-theme-resolved', resolvedTheme);
  } catch (e) {}
}

async function saveThemeConfig() {
  const themeSelect = document.getElementById('app-theme-select');
  const autoTypeSelect = document.getElementById('theme-auto-type');
  const batteryThresh = document.getElementById('theme-battery-thresh');
  const schedLight = document.getElementById('theme-sched-light');
  const schedDark = document.getElementById('theme-sched-dark');

  if (!themeSelect) return;

  const config = {
    theme: themeSelect.value,
    autoType: autoTypeSelect ? autoTypeSelect.value : 'system',
    batteryThreshold: batteryThresh ? parseInt(batteryThresh.value) || 20 : 20,
    scheduleLightStart: schedLight ? schedLight.value || '08:00' : '08:00',
    scheduleDarkStart: schedDark ? schedDark.value || '20:00' : '20:00'
  };

  await dbSetMeta('themeConfig', config);
  try {
...
function updateThemeOptionsVisibility() {
  const themeSelect = document.getElementById('app-theme-select');
  const autoOptions = document.getElementById('theme-auto-options');
  const autoTypeSelect = document.getElementById('theme-auto-type');
  const optBattery = document.getElementById('theme-opt-battery');
  const optSchedule = document.getElementById('theme-opt-schedule');

  if (!themeSelect || !autoOptions) return;

  if (themeSelect.value === 'auto') {
    autoOptions.style.display = 'flex';
    if (autoTypeSelect.value === 'battery') {
      optBattery.style.display = 'flex';
      optSchedule.style.display = 'none';
    } else if (autoTypeSelect.value === 'schedule') {
      optBattery.style.display = 'none';
      optSchedule.style.display = 'grid';
    } else {
      optBattery.style.display = 'none';
      optSchedule.style.display = 'none';
    }
...
async function initSettings() {
  ...
  document.getElementById('github-save-settings').addEventListener('click', saveGithubSettings);
  document.getElementById('github-push').addEventListener('click', pushToGithub);
  document.getElementById('github-pull').addEventListener('click', pullFromGithub);

  // Botones de modal descifrado
  document.getElementById('github-decrypt-submit').addEventListener('click', submitGithubDecrypt);
  document.getElementById('github-decrypt-skip').addEventListener('click', skipGithubDecrypt);
  ...
  if (themeSelect) {
    ...
    if (autoTypeSelect) autoTypeSelect.value = config.autoType || 'system';
    if (batteryThresh) batteryThresh.value = config.batteryThreshold || 20;
    if (schedLight) schedLight.value = config.scheduleLightStart || '08:00';
    if (schedDark) schedDark.value = config.scheduleDarkStart || '20:00';
    ...
    if (autoTypeSelect) autoTypeSelect.addEventListener('change', handleThemeChange);
    if (batteryThresh) batteryThresh.addEventListener('input', saveThemeConfig);
    if (schedLight) schedLight.addEventListener('change', saveThemeConfig);
    if (schedDark) schedDark.addEventListener('change', saveThemeConfig);
  }
}
...
async function restoreFromPayload(payload) {
  ...
  // Restaurar historial si viene en el payload
  if (incoming.history.length) {
    await dbClearHistory();
    for (const h of incoming.history) await dbAddHistory(h);
  }
  ...
  if (payload.meta) {
    ...
    if (payload.meta.themeConfig) {
      await dbSetMeta('themeConfig', payload.meta.themeConfig);
      try {
        localStorage.setItem('app-theme-config', JSON.stringify(payload.meta.themeConfig));
      } catch (e) {}
      await updateActiveTheme();
      ...
      if (themeSelect) themeSelect.value = payload.meta.themeConfig.theme || 'auto';
      if (autoTypeSelect) autoTypeSelect.value = payload.meta.themeConfig.autoType || 'system';
      if (batteryThresh) batteryThresh.value = payload.meta.themeConfig.batteryThreshold || 20;
      if (schedLight) schedLight.value = payload.meta.themeConfig.scheduleLightStart || '08:00';
      if (schedDark) schedDark.value = payload.meta.themeConfig.scheduleDarkStart || '20:00';
      updateThemeOptionsVisibility();
    } else if (payload.meta.theme) {
      await applyTheme(payload.meta.theme);
      const themeSelect = document.getElementById('app-theme-select');
      if (themeSelect) themeSelect.value = payload.meta.theme;
      updateThemeOptionsVisibility();
    }
  }
...
async function exportData() {
  try {
    const [taskList, catList, projList, histList, orKey, makerCats, savedThemeConfig] = await Promise.all([
      dbGetAll(), dbGetAllCats(), dbGetAllProjects(), dbGetHistory(),
      dbGetMeta('openRouterKey'), dbGetMeta('makerCategories'), dbGetMeta('themeConfig')
    ]);
    const payload = {
      version: 1,
      ...
      meta: {
        ...
        themeConfig: savedThemeConfig || null
      }
    };
...
// ── Sincronización con GitHub (Cifrado Local Cliente) ──
let githubSettings = null;

async function initGithubSync() {
  try {
    const settings = await dbGetMeta('github_settings');
    if (!settings) {
    ...
    if (storedPass && savedAtStr) {
      ...
      if (!isExpired) {
        try {
          ...
          console.warn('[GitHubSync] Error al descifrar automáticamente con clave de localStorage en Tareas:', decryptErr);
        }
      } else {
        ...
        const skipBtn = document.getElementById('github-decrypt-skip');
    ...
    console.error('[GitHubSync] Error al inicializar:', e);
  }
}
...
function fillGithubUIFields() {
  ...
  if (pathInput) pathInput.value = githubSettings.filepath || 'daily-planner-backup.json';
...
async function submitGithubDecrypt() {
  ...
  try {
    ...
    showToast('Credenciales descifradas correctamente');
    updateGithubUIState('connected');
    fillGithubUIFields();
...
async function saveGithubSettings() {
  ...
  const pathVal = pathInput ? pathInput.value.trim() : 'daily-planner-backup.json';
  ...
  try {
    ...
    await dbSetMeta('github_settings', settingsObj);
...
async function pushToGithub() {
  ...
  if (pushBtn) {
    ...
    pushBtn.innerHTML = 'Subiendo respaldo...';
  }
  ...
  try {
    ...
    const newSha = await uploadBackupRemote(
      token,
      githubSettings.repo,
    ...
    showToast('☁ Copia de seguridad subida a GitHub');
  } catch (e) {
...
function pullFromGithub() {
  if (githubSettings) {
    const token = sessionStorage.getItem('github_token');
    if (!token) {
      showToast('Descifra tus credenciales primero.');
      return;
    }
    verifyCloudVersionSilently(token, true);
  }
}

async function verifyCloudVersionSilently(token, forcePrompt = false) {
  if (!githubSettings) return;
  ...
  try {
    ...
    if (!meta.content) {
      if (forcePrompt) {
        showToast('No se encontró archivo de respaldo en GitHub.');
        setCloudStatus('Sin backup remoto', '');
      }
      return;
    }
    ...
    if (forcePrompt || (cloudTime > localTime + 5000)) {
      ...
      showSyncModal(backupData);
    } else {
      const t = new Date(cloudTime);
      const timeStr = t.toLocaleString('es', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' });
      setCloudStatus('Sincronizado · ' + timeStr, 'ok');
    }
  } catch (e) {
    ...
    if (forcePrompt) {
      showToast('Error al descargar copia: ' + e.message);
      setCloudStatus('Error al conectar', 'err');
    }
  }
}
...
function showSyncModal(remote) {
  ...
  const dateStr = new Date(remote.updatedAt).toLocaleString('es', {
  ...
  document.getElementById('sync-keep-local').onclick = async () => {
    ...
    showToast('Datos locales conservados. Se actualizará la nube al respaldar.');
  };
}

function setCloudStatus(msg, type) {
  const txt = document.getElementById('github-status');
...
function updateGithubUIState(state) {
  ...
  if (state === 'unconfigured') {
    setCloudStatus('Sin configurar', '');
    if (actions) actions.style.display = 'none';
  } else if (state === 'locked') {
    setCloudStatus('Bloqueado (Requiere contraseña)', 'err');
    if (actions) actions.style.display = 'none';
  } else if (state === 'connected') {
    setCloudStatus('Conectado a GitHub', 'ok');
    if (actions) actions.style.display = 'flex';
  } else if (state === 'local') {
    setCloudStatus('Desconectado (Modo local)', '');
    if (actions) actions.style.display = 'none';
  }
}
...
// ══════════════════════════════════════════
// HISTORIAL — Inicialización y migración
// ══════════════════════════════════════════

function initHistory() {
  const navBtn = document.querySelector('.nav-fab-item[data-nav="historial"]');
  if (navBtn) {
    navBtn.addEventListener('click', () => {
      closeNav();
...
/**
 * Migración retroactiva (una sola vez, controlada por meta 'historyMigrated').
 * Genera entradas de historial a partir de las tareas ya existentes en la BD:
 *   - 'migrated'  → fecha de creación inferida por task.id (timestamp)
 *   - 'completed' → si task.done === true
 */
async function migrateHistoryRetroactive() {
  try {
    const alreadyDone = await dbGetMeta('historyMigrated');
    if (alreadyDone) return;

    const allTasks = await dbGetAll();
    if (!allTasks.length) {
      await dbSetMeta('historyMigrated', true);
      return;
    }

    const sorted = [...allTasks].sort((a, b) => a.id - b.id);
    ...
    for (const t of sorted) {
      const createEntry = {
        ...
        taskText: t.text || '',
        taskPri:  t.pri  || '',
        cats:     Array.isArray(t.cats) ? t.cats : [t.cat].filter(Boolean),
        ts:       new Date(t.id).toISOString(),
      };
      await dbAddHistory(createEntry);

      if (t.done) {
        const doneTs = t.doneAt
          ? new Date(t.doneAt + 'T12:00:00').getTime()
          : t.id + 1;
        const doneEntry = {
          id:       doneTs + Math.floor(Math.random() * 999),
          type:     'completed',
          taskId:   t.id,
          taskText: t.text || '',
          taskPri:  t.pri  || '',
          cats:     Array.isArray(t.cats) ? t.cats : [t.cat].filter(Boolean),
          ts:       new Date(doneTs).toISOString(),
        };
        await dbAddHistory(doneEntry);
      }
    }

    await dbSetMeta('historyMigrated', true);
  } catch (err) {
    console.warn('migrateHistoryRetroactive error:', err);
  }
}
